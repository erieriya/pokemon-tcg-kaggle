"""
PTCG AI Battle Challenge - Archaludon ex + Cinderace ヒューリスティック対戦相手

Kaggle Notebook "A sample Archaludon 75% WR vs my 1300 Starmie"
(https://www.kaggle.com/code/masamikobayashi/a-sample-archaludon-75-wr-vs-my-1300-starmie)
の移植。

Cinderace の Turbo Flare と Archaludon ex の Assemble Alloy による
メタルエネルギー加速を主軸にした、既存の Mega Lucario ex 系とは異なる
アーキタイプを固定対戦相手として追加する。

移植時の変更点:
- Kaggle 専用の sys.path 操作と /kaggle_simulations/agent 参照を削除。
- deck.csv 読み込み関数を deck_archaludon.csv 参照に変更。
...
def read_deck_csv() -> list[int]:
    file_path = os.path.join(os.path.dirname(__file__), "deck_archaludon.csv")
