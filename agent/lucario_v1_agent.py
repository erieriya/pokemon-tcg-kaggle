"""
PTCG AI Battle Challenge - Lucario v1 ヒューリスティック対戦相手

Kaggle Notebook "Pokemon AI Battle Agent: Mega Lucario"
(https://www.kaggle.com/code/nursrijan/pokemon-ai-battle-agent-mega-lucario)
の LucarioPolicy をPPO自己対戦の固定対戦相手として移植したもの。

元notebookには前向き探索(search_begin等)のオプション機能があったが、
このリポジトリのcgライブラリのsearch_begin署名は notebook が想定する
ものと異なり(デッキ/手札の予測情報を要求する)動かないため、移植では
greedyなスコアリング方策のみを採用している(元notebookもUSE_SEARCH失敗時は
このgreedy方策にフォールバックする設計だった)。

元notebookのデッキは合計61枚(基本格闘エネルギーが14枚)というバグがあり、
60枚制約を満たさないため、deck_lucario_v1.csv では基本格闘エネルギーを
1枚減らして60枚に修正している。
"""
...
def read_deck_csv() -> list[int]:
    file_path = os.path.join(os.path.dirname(__file__), "deck_lucario_v1.csv")
    if not os.path.exists(file_path):
        file_path = "/kaggle_simulations/agent/deck_lucario_v1.csv"
