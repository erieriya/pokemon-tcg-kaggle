...
# rl_agent.py のモジュールimport経由とは異なり、このファイル自体が main.py として
# exec される場合は try/except で /kaggle_simulations/agent/ にフォールバックする。
# これにより、関数内の遅延 `from cg.api import ...` が確実に解決される。
try:
