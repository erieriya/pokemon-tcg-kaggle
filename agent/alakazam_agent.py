"""
PTCG AI Battle Challenge - Alakazam ヒューリスティック対戦相手

Kaggle Notebook "Rule-based not Psychic Alakazam best 5th"
(https://www.kaggle.com/code/ryotasueyoshi/rule-based-not-psychic-alakazam-best-5th)
の移植。

Alakazam の Powerful Hand(手札枚数に応じたダメージ)を主軸に、
Kadabra/Alakazam、Dudunsparce、Fezandipiti ex で手札を増やす
非 Psychic 型のアーキタイプを固定対戦相手として追加する。

移植時の変更点:
- Kaggle 専用の sys.path 操作と /kaggle_simulations/agent 参照を削除。
- モジュール import 時の deck.csv 読み込みを削除し、obs.select is None の互換返却時だけ
  deck_alakazam.csv を読む read_deck_csv() に変更。
- cg へのパス追加は train_ppo.py 側に任せ、from cg.api import ... の素の import に統一。
"""
...
def read_deck_csv() -> list[int]:
    file_path = os.path.join(os.path.dirname(__file__), "deck_alakazam.csv")
